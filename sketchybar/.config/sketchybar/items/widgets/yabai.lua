...
-- Метод для динамической переупаковки порядка элементов на панели
...
local function refresh_workspace()
	-- 0.05s задержка даёт yabai завершить внутренние изменения перед запросом
	sbar.delay(0.05, function()
		...
		sbar.exec(spaces_cmd, function(spaces_output)
			...
			-- Сбрасываем отображение всех слотов спейсов
			for i = 1, MAX_SPACES do
